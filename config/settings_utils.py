...
HYPRIDLE_HEADER = f"# Generated by {APP_NAME_CAP}. Set the idle timeout in {APP_NAME_CAP} settings."
# Apply restarts hypridle while the Hyprland reload hook also starts it; both
# can pass a "not running" check at once, so hold a lock to allow only one.
# -w (not -n) lets a restart wait for the killed instance to release it.
HYPRIDLE_START = f"uwsm app -- flock -w 3 $XDG_RUNTIME_DIR/{APP_NAME}-hypridle.lock hypridle"
...
def apply_and_restart(replace_lock: bool = False, replace_idle: bool = False) -> None:
    ...
    idle_src = write_hypridle()
    idle_dest = CONFIG_DIR / "hypr" / "hypridle.conf"
    if replace_idle:
        backup_and_replace(idle_src, idle_dest, "Hypridle")
        restart_hypridle()
    elif is_aw_hypridle(idle_dest) and idle_dest.read_text(encoding="utf-8") != idle_src.read_text(encoding="utf-8"):
        # Installed from Aw-Shell: keep it in sync with the timeout setting
        shutil.copy(idle_src, idle_dest)
        restart_hypridle()
...
hl.on("config.reloaded", function()
    hl.exec_cmd({lua_str(f"pgrep -x hypridle > /dev/null || {HYPRIDLE_START}")})
...
def write_hypridle() -> Path:
    """Write the generated hypridle.conf into the Aw-Shell config dir."""
    path = AW_CONFIG_DIR / "hypr" / "hypridle.conf"
    path.parent.mkdir(parents=True, exist_ok=True)
    try:
        path.write_text(generate_hypridle(), encoding="utf-8")
    except Exception as e:
        print(f"Error writing hypridle config: {e}")
    return path


def is_aw_hypridle(path: Path) -> bool:
    """True if `path` is a hypridle.conf generated by Aw-Shell."""
    try:
        return path.read_text(encoding="utf-8").startswith(HYPRIDLE_HEADER)
    except OSError:
        return False


def restart_hypridle() -> None:
    """Restart hypridle so it picks up config changes."""
    try:
        subprocess.run(["pkill", "-x", "hypridle"], capture_output=True)
        subprocess.Popen(
            HYPRIDLE_START, shell=True,
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
            start_new_session=True,
        )
    except Exception as e:
